...
`java -jar <path_to>/jactl-VERSION.jar` every time.
...
```shell
#!/bin/bash
java -jar <path_to>/jactl-VERSION.jar "$@"
```

Note that the `/bin/bash` should be the location of your shell and `<path_to>` should be replaced with the location
of the `jactl-VERSION.jar` file.

We will assume from now on that you have such a shell script and will use `jactl` in place of `java -jar jactl-VERSION.jar`. 
...
|:--------------------|:-------|:---------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `environmentClass`  | String | `io.jactl.DefaultEnv` | The name of the class which will is used to encapsulate the Jactl runtime environment. See [Integration Guide](integration-guide/introduction) for more details.                                                                              |
...
If the jars are located under `~/projects/jactl-vertx/build/libs` then a `.jactlrc` file that allows
...
extraJars        = [ '~/projects/jactl-vertx/build/libs/jactl-vertx-VERSION.jar',
                     '~/projects/jactl-vertx/build/libs/jactl-vertx-VERSION-tests.jar' ]
...
```groovy
def VERSION = 'VERSION'     // The jactl-vertx version to use
