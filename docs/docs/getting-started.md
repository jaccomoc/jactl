...
[https://repo1.maven.org/maven2/io/jactl/jactl/2.7.2/jactl-2.7.2.jar](https://repo1.maven.org/maven2/io/jactl/jactl/2.7.2/jactl-2.7.2.jar)

Then, to run a script you can use `-e` to specify the script on the command line:
```shell
java -jar jactl-2.7.2.jar -e '10.map{ it + 1 }.sum()'
```
Or, you can provide the name of a file that contains the script:
```shell
java -jar jactl-2.7.2.jar myscript.jactl
```
...
For example:
```shell
jbang run io.jactl:jactl:2.7.2 -e '10.map{ it + 1 }.sum()'
```

```shell
jbang run io.jactl:jactl:2.7.2 myscript.jactl
```
...
To get the JAR, download it from [https://repo1.maven.org/maven2/io/jactl/jactl-repl/2.7.2/jactl-repl-2.7.2.jar](https://repo1.maven.org/maven2/io/jactl/jactl-repl/2.7.2/jactl-repl-2.7.2.jar)
and then run it directly:
```shell
$ java -jar jactl-repl-2.7.2.jar
> 10.map{ it + 1 }.sum()
55
```
...
```shell
$ jbang run io.jactl:jactl-repl:2.7.2
[jbang] Resolving dependencies...
[jbang]    io.jactl:jactl-repl:2.7.2
...
```groovy
implementation group: 'io.jactl', name: 'jactl', version: '2.7.2'
```
</TabItem>
