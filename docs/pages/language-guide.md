...
These are similar to `static final` fields in Java.
For example:
```groovy
class Math {
...
The `import static` directive also works for class constants defined in another class:
```groovy
...
Constants declared with the `const` keyword can be used in a `switch` expression pattern and work the same way as
