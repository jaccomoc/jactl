...
The benchmarks were generated by [Claude](https://claude.ai) and are modelled on the [camel-jmh](https://github.com/apache/camel-performance-tests/tree/main/tests/camel-jmh)
...
Jactl leads at **94.5M evaluations/s**, 16.2x faster than Groovy (5.83M) and **57.7x** faster than
Simple (1.64M).
...
with (46.9M evaluations/s) being 8.1x faster than Groovy.
Jactl still comes out ahead at **99.1M evaluations/s**, just over twice as fast as Simple and **17.1x**
faster than Groovy.
...
Jactl is fastest at **76.1M evaluations/s**, 12.8x faster than Groovy (5.96M) and **25.9x**
faster than Simple (2.93M).
...
Jactl completes **1.25M route invocations/s** (each invocation is two exchanges),
1.7x ahead of Groovy (734K) and 2.0x ahead of Simple (632K).
...
At 10 line items Jactl evaluates the summary **2.64M times/s**, **3.7x** faster than
Groovy (723K).
At 200 line items Jactl's advantage grows to **5.0x** (240K vs 48.2K evaluations/s).
...
Here Jactl is **7.0x** faster at 10 items (8.82M vs 1.25M evaluations/s) and **5.0x** faster
at 200 items (466K vs 92.3K).
...
At 10 line items the plain-Java route manages **630K invocations/s**, with Jactl at
**552K** (88% of the Java baseline) and **2.8x** faster than Groovy (200K, or 32% of Java).

At 200 line items, Jactl holds **69%** of the Java baseline (105K vs 152K) and is **4.1x** faster
than Groovy (25.5K, 17% of Java).
...
Jactl can compile and evaluate **29.8K unique predicates per second** which is **9.3x** more than
Groovy's 3.21K, even though both languages are generating and loading real JVM bytecode.
...
| Header predicate               |  **17.1x** | _baseline_ |       8.1x |        - |
| Value expression               |  **25.9x** |       2.0x | _baseline_ |        - |
| Filter route (end-to-end)      |   **2.0x** |       1.2x | _baseline_ |        - |
| Summary script (10 items)      |   **3.7x** | _baseline_ |          - |        - |
| Summary script (200 items)     |   **5.0x** | _baseline_ |          - |        - |
| Express predicate (10 items)   |   **7.0x** | _baseline_ |          - |        - |
| Express predicate (200 items)  |   **5.0x** | _baseline_ |          - |        - |
| Order route (10 items)         |       2.8x | _baseline_ |          - | **3.2x** |
...
The [Jactl for Groovy Programmers Guide](/docs/jactl-for-groovy-programmers) provides a quick overview of the Jactl language for programmers already familiar with Groovy.
...
> This post was co-authored with [Claude](https://claude.ai).
