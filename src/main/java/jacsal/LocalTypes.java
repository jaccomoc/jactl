...
public class LocalTypes {
  ...
  public void push(JacsalType type) {
    stack.push(new StackEntry(type));
  }

  public JacsalType pop() {
    var entry = stack.pop();
    freeSlot(entry.slot);
    return entry.type;
  }
  ...
  public void setStackType(JacsalType type) {
  ...
  private void _popVal(JacsalType type) {
   ...
   */
  public int allocateSlot(JacsalType type) {
    int i;
   ...
   * @return the slot
   */
  public int allocateSlot(JacsalType type, boolean isParam) {
  ...
  private void _loadLocal(int slot, JacsalType type) {
    if (type.isPrimitive()) {
      switch (type.getType()) {
