...
//    compiler.compileClass();
  ...
  @Override public Void visitRegexSubst(Expr.RegexSubst expr) {
    ...
    // Create a StringBuffer to store result into
    _newInstance(StringBuffer.class);
  ...
  private static final Map<TokenType,List<Object>> opCodesByOperator = Utils.mapOf(
    PLUS, Utils.listOf(Utils.listOf(IADD), Utils.listOf(LADD), Utils.listOf(DADD), RuntimeUtils.PLUS),
    MINUS, Utils.listOf(Utils.listOf(ISUB), Utils.listOf(LSUB), Utils.listOf(DSUB), RuntimeUtils.MINUS),
    STAR, Utils.listOf(Utils.listOf(IMUL), Utils.listOf(LMUL), Utils.listOf(DMUL), RuntimeUtils.STAR),
    SLASH, Utils.listOf(Utils.listOf(IDIV), Utils.listOf(LDIV), Utils.listOf(DDIV), RuntimeUtils.SLASH),
    PERCENT_PERCENT, Utils.listOf(Utils.listOf(IREM), Utils.listOf(LREM), Utils.listOf(DREM), RuntimeUtils.PERCENT_PERCENT),
    AMPERSAND, Utils.listOf(Utils.listOf(IAND), Utils.listOf(LAND), Utils.listOf(), Utils.listOf()),
    PIPE, Utils.listOf(Utils.listOf(IOR), Utils.listOf(LOR), Utils.listOf(), Utils.listOf()),
    ACCENT, Utils.listOf(Utils.listOf(IXOR), Utils.listOf(LXOR), Utils.listOf(), Utils.listOf()),
    DOUBLE_LESS_THAN, Utils.listOf(Utils.listOf(ISHL), Utils.listOf(LSHL), Utils.listOf(), Utils.listOf()),
    DOUBLE_GREATER_THAN, Utils.listOf(Utils.listOf(ISHR), Utils.listOf(LSHR), Utils.listOf(), Utils.listOf()),
    TRIPLE_GREATER_THAN, Utils.listOf(Utils.listOf(IUSHR), Utils.listOf(LUSHR), Utils.listOf(), Utils.listOf()),
    PERCENT, Utils.listOf(Utils.listOf(DUP_X1, IREM, SWAP, DUP_X1, IADD, SWAP, IREM),
                               Utils.listOf(DUP2_X2, LREM,  DUP2_X2, POP2, DUP2_X2, LADD, DUP2_X2, POP2, LREM),
                               Utils.listOf(DUP2_X2, DREM,  DUP2_X2, POP2, DUP2_X2, DADD, DUP2_X2, POP2, DREM),
...
//                 });
  ...
  @Override public Void visitListLiteral(Expr.ListLiteral expr) {
    if (expr.isConst) {
      loadClassConstant(expr.constValue);
      return null;
    }
  ...
  @Override public Void visitIdentifier(Expr.Identifier expr) {
    ...
    final String  name              = expr.identifier.getStringValue();
    final boolean isBuiltinFunction = expr.varDecl.funDecl != null && expr.varDecl.funDecl.functionDescriptor.isBuiltin;
    ...
    if (expr.createIfMissing) {
      ...
      if (expr.type.is(INSTANCE) && expr.type.getJactlClassDescriptor().allFieldsAreDefaults() && !expr.createAsList ||
  ...
  private void invokeAndCatchRuntimeException(Class<?> hostClass, Executable m, JactlType returnType, Token location) {
    tryCatch(RuntimeException.class, false,
             () -> invokeMethod(hostClass, m, returnType),
             () -> {
               int temp = stack.saveInTemp();
               mv.visitTypeInsn(NEW, "io/jactl/runtime/RuntimeError");
               mv.visitInsn(DUP);
               if (m instanceof Constructor) {
                 _loadConst("Error invoking constructor for host class '" + m.getDeclaringClass().getName() + "'");
               }
               else {
                 _loadConst("Error invoking host class method '" + m.getDeclaringClass().getName() + "." + m.getName() + "()'");
               }
    ...
    if (args.size() == method.paramCount && !Utils.isNamedArgs(args) && validateArgsAtCompileTime) {
      ...
      invokeMaybeAsync(isAsync, method.returnType, 0, location,
                       () -> {
                         ...
                         else if (method.isStaticImplementation && !parent.type.is(CLASS)) {
                           popVal();
                         }
  ...
  @Override public Void visitConvertTo(Expr.ConvertTo expr) {
    ...
    LocalLocation location   = new LocalLocation(sourceSlot, offsetSlot);
    compile(expr.expr);
  ...
  private void convertToInstance(JactlType varType, Expr expr, Location compileTimeLocation, SourceLocation runtimeLocation) {
    ...
    FunctionDescriptor method = varType.isHostClass() || varType.getJactlClassDescriptor() == null 
                                ? null : varType.getJactlClassDescriptor().getMethod(Utils.JACTL_INIT);
    
    if (method == null && !valueType.is(ANY)) {
    ...
    invokeMaybeAsync(method != null && method.isAsync(), ANY, 0, compileTimeLocation,
                     () -> {
                       if (valueType.is(ANY)) {
                         ...
                         _loadLocal(temp);
                         mv.visitJumpInsn(IFNULL, skipInit);  // null is allowed as a valid value
                         _loadLocal(temp);
                         mv.visitTypeInsn(INSTANCEOF, varType.getInternalName());
                         mv.visitJumpInsn(IFNE, skipInit);
                         if (method == null) {
    ...
    mv.visitLabel(skipInit);                      // :noInitMethod
    loadLocal(temp);
    checkCast(varType);
    stack.freeSlot(temp);
  }
  ...
  private void compileFieldAccess(Expr.Binary expr) {
    ...
    ArrayList<Expr.Binary> nodes = new ArrayList<Expr.Binary>();
    for (Expr node = expr;
    ...
    for (ListIterator<Expr.Binary> iter = nodes.listIterator(nodes.size()); iter.hasPrevious(); ) {
      ...
      JactlClassDescriptor parentClass = node.left.type.getJactlClassDescriptor();
      JactlType            childType   = parentClass.getField(fieldName);
      // Can only create if createIfMissing flag set and if there are no mandatory fields (i.expr. init method
      // has 0 parameters).
      boolean createIfMissing = node.createIfMissing && !(childType.is(INSTANCE) && childType.getJactlClassDescriptor().getInitMethod().paramCount > 0);
  ...
  private boolean validateArgs(List<Expr> args, FunctionDescriptor func, Token location, boolean isInitMethod, JactlType initClass) {
    ...
    List<JactlType> paramTypes = func.paramTypes;
  ...
  private void validateNamedArgs(List<Expr> args, FunctionDescriptor func, Token location, boolean isInitMethod, JactlType initClass) {
    ...
    List<String>    paramNames = isInitMethod ? initClass.getClassDescriptor().getAllFieldNames() : func.paramNames;
    List<JactlType> paramTypes = isInitMethod ? initClass.getClassDescriptor().getAllFieldTypes() : func.paramTypes;
    ...
    for (int i = 0; i < paramNames.size(); i++) {
      ...
      JactlType paramType = paramTypes.get(i);
      Expr      argExpr   = namedArgMap.get(paramName);
      if (argExpr == null) {
  ...
  private void validateArg(Expr argExpr, JactlType paramType, Token location) {
    if (argExpr.type.is(MAP) && paramType.is(INSTANCE) && !paramType.isHostClass()) {
      ...
      if (argExpr.isConstMap()) {
        validateNamedArgs(Utils.listOf(argExpr), initMethod, argExpr.location, true, paramType);
      }
    }
    else
  ...
  private void newInstance(JactlType type) {
    newInstance(type, 0, null, null, null);
  }

  private void newInstance(JactlType type, int numDimensionsSepcified, Expr.MethodCall callInit, List<Expr> args, Token location) {
    if (type.is(ARRAY)) {
      ...
      if (type.isHostClass()) {
        // For host class instances we have to call the actual constructor with any args
        Class<?> javaClass = type.getJavaClass();
        Constructor constructor = classCompiler.context.findMatchingConstructor(javaClass, args, location);
        
        // If we have any async args we need to evaluate them first because we can't save the state
        ...
        if (asyncArgs) {
          ...
          List<Integer> argSlots = IntStream.range(0, args.size()).map(i -> stack.saveInTemp()).boxed().collect(Collectors.toList());
          _newInstance(type.getInternalName(), type.isHostClass());
          pushType(type);
          dupVal();
          // Restore args using argSlots in reverse order
          for (int i = argSlots.size() - 1; i >= 0; i--) {
            loadLocal(argSlots.get(i));
          }
          invokeAndCatchRuntimeException(javaClass, constructor, VOID, location);
        }
        else {
          _newInstance(type.getInternalName(), type.isHostClass());
          pushType(type);
  ...
  private void _newInstance(Class clss) {
    _newInstance(clss, false);
  }
  
  private void _newInstance(Class clss, boolean isHostClass) {
    _newInstance(Type.getInternalName(clss), isHostClass);
  }

  private void _newInstance(String className) {
    _newInstance(className, false);
  }
  
  private void _newInstance(String className, boolean isHostClass) {
    mv.visitTypeInsn(NEW, className);
    if (!isHostClass) {
      mv.visitInsn(DUP);
      // Invoke <init> if not a host class. For host classes we invoke <init> in visitMethodCall.
  ...
  private void pushType(Class clss) {
    stack.push(getTypeFromClass(clss));
  }

  private JactlType getTypeFromClass(Class clss) {
    return classCompiler.context.typeFromClass(clss);
  }
  ...
  private void throwNullError(String msg, SourceLocation location) {
    ...
    _loadConst(msg);
    _loadLocation(location);
    // Don't capture stack traces if wrapped in a try/catch(NullError)
    boolean captureStack = !insideTryCatchNullError();
    _loadConst(captureStack);
    mv.visitMethodInsn(INVOKESPECIAL, "io/jactl/runtime/NullError", "<init>", "(Ljava/lang/String;Ljava/lang/String;IZ)V", false);
    mv.visitInsn(ATHROW);
  }
  ...
  /**
   * Throw RuntimeError with given message prepended to string that is on stack.
   * String on stack may be null in which case we don't concatenate it.
   * <p>Expect stack: ...,string</p>
   * @param msg       message to prepend to string
   * @param location
   */
  private void throwErrorWithStringOrNull(String msg, SourceLocation location) {
    _loadConst(msg);
    _loadLocation(location);
    mv.visitMethodInsn(INVOKESTATIC, "io/jactl/runtime/RuntimeUtils", RuntimeUtils.ERROR_WITH_MSG,
                       "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;I)Lio/jactl/runtime/RuntimeError;", false);
    mv.visitInsn(ATHROW);
  }
  ...
  private void tryCatch(Class exceptionClass, boolean checkStacksMatch, Runnable tryBlock, Runnable catchBlock) {
    ...
    LocalTypes preTryState = stack.copy();
    tryBlock.run();
...
//                   getMethodDescriptor(functionDescriptor.returnType, paramTypes));
  ...
  private void loadDefaultValueOrNewInstance(JactlType type, Token location) {
    if (type.is(INSTANCE)) {
      ...
      FunctionDescriptor initMethod = type.getJactlClassDescriptor().getInitMethod();
  ...
  /**
   * Load list/array element or string char at given position.
