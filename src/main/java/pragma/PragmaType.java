...
import pragma.runtime.Continuation;
import pragma.runtime.HeapLocal;
import pragma.runtime.RegexMatcher;
...
public class PragmaType {
  ...
  public static PragmaType BOOLEAN       = createPrimitive(TypeEnum.BOOLEAN);
  public static PragmaType BOXED_BOOLEAN = createBoxedType(TypeEnum.BOOLEAN);
  public static PragmaType INT           = createPrimitive(TypeEnum.INT);
  public static PragmaType BOXED_INT     = createBoxedType(TypeEnum.INT);
  public static PragmaType LONG          = createPrimitive(TypeEnum.LONG);
  public static PragmaType BOXED_LONG    = createBoxedType(TypeEnum.LONG);
  public static PragmaType DOUBLE        = createPrimitive(TypeEnum.DOUBLE);
  public static PragmaType BOXED_DOUBLE  = createBoxedType(TypeEnum.DOUBLE);
  public static PragmaType DECIMAL       = createRefType(TypeEnum.DECIMAL);
  public static PragmaType STRING        = createRefType(TypeEnum.STRING);
  public static PragmaType MAP           = createRefType(TypeEnum.MAP);
  public static PragmaType LIST          = createRefType(TypeEnum.LIST);
  public static PragmaType ANY           = createRefType(TypeEnum.ANY);
  public static PragmaType FUNCTION      = createRefType(TypeEnum.FUNCTION);

  public static PragmaType HEAPLOCAL    = createRefType(TypeEnum.HEAPLOCAL);
  public static PragmaType OBJECT_ARR   = createRefType(TypeEnum.OBJECT_ARR);
  public static PragmaType LONG_ARR     = createRefType(TypeEnum.LONG_ARR);
  public static PragmaType STRING_ARR   = createRefType(TypeEnum.STRING_ARR);
  public static PragmaType ITERATOR     = createRefType(TypeEnum.ITERATOR);
  public static PragmaType MATCHER      = createRefType(TypeEnum.MATCHER);
  public static PragmaType CONTINUATION = createRefType(TypeEnum.CONTINUATION);
  public static PragmaType INSTANCE     = createRefType(TypeEnum.INSTANCE);
  ...
  private PragmaType(TypeEnum type, boolean boxed, boolean isRef) {
    this.type = type;
    this.boxed = boxed;
    this.isRef = isRef;
  }

  private static PragmaType createPrimitive(TypeEnum type) {
    return new PragmaType(type, false, false);
  }
  private static PragmaType createBoxedType(TypeEnum type) {
    return new PragmaType(type, true, true);
  }
  private static PragmaType createRefType(TypeEnum type) {
    return new PragmaType(type, false, true);
  }

  public static PragmaType createInstance(String internalClassName) {
    PragmaType type = createRefType(TypeEnum.INSTANCE);
  ...
  /**
   * Create a PragmaType from a TokenType
   */
  public static PragmaType valueOf(TokenType tokenType) {
    switch (tokenType) {
  ...
  /**
   * Return the TokenType that would generate this PragmaType
   */
   ...
   */
  public boolean is(PragmaType... types) {
    for (PragmaType type: types) {
      if (this == type) {
        return true;
      }
    }
   ...
   * is boxed).
   */
  public boolean isBoxedOrUnboxed(PragmaType... types) {
    for (PragmaType type: types) {
  ...
  private static class TypePair {
    PragmaType t1; PragmaType t2;
    TypePair(PragmaType t1, PragmaType t2) { this.t1 = t1; this.t2 = t2; }
  ...
  private static final Map<TypePair, PragmaType> resultMap   = new HashMap<>();
  static {
    for (Iterator it = resultTypes.iterator(); it.hasNext(); ) {
      resultMap.put((TypePair)it.next(), (PragmaType)it.next());
    }
  }
   ...
   */
  public static PragmaType result(PragmaType type1, Token operator, PragmaType type2) {
    // Boolean comparisons
    ...
    if (operator.is(EQUAL,QUESTION_COLON,QUESTION)) {
      ...
      if (operator.is(QUESTION)) {
        PragmaType result = resultType(type1, type2);
        if (result != null) {
          return result;
        }
  ...
  private static PragmaType resultType(PragmaType type1, PragmaType type2) {
    if (type1.is(type2)) {
      return type1;
    }
   ...
   */
  public boolean isConvertibleTo(PragmaType type) {
  ...
  public String descriptor() {
    switch (this.type) {
      ...
      case HEAPLOCAL:      return Type.getDescriptor(pragma.runtime.HeapLocal.class);
  ...
  public static PragmaType typeOf(Object obj) {
  ...
  public static PragmaType typeFromClass(Class clss) {
  ...
  private static void checkIsNumeric(PragmaType type, String leftOrRight, Token operator) {
