...
import static pragma.PragmaType.FUNCTION;
import static pragma.TokenType.*;

/**
 * Recursive descent parser for parsing the Pragma language.
...
public class Parser {
  ...
  public Stmt.ClassDecl parse() {
    Token start = peek();
    Stmt.ClassDecl scriptClass = new Stmt.ClassDecl(new Token(IDENTIFIER, start).setValue(Utils.PRAGMA_PREFIX));
  ...
  private Stmt.FunDecl script() {
    Token start = peek();
    Token scriptName = new Token(IDENTIFIER, start).setValue(Utils.PRAGMA_SCRIPT_MAIN);
    // Script take a single parameter which is a Map of globals
    Token        name     = new Token(IDENTIFIER, start).setValue(Utils.PRAGMA_GLOBALS_NAME);
    Expr.VarDecl declExpr = new Expr.VarDecl(name, null);
    declExpr.type = PragmaType.MAP;
  ...
  private Stmt.FunDecl funDecl() {
    ...
    PragmaType returnType      = PragmaType.valueOf(returnTypeToken.getType());
  ...
  private Stmt.VarDecl singleVarDecl(Token typeToken) {
    ...
    if (typeToken.is(VAR)) {
      ...
      // Convert token to a PragmaType
      type = typeToken.is(DEF) ? ANY
                               : PragmaType.valueOf(typeToken.getType());
    }
