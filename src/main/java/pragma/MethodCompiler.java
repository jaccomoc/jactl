...
import static pragma.PragmaType.BOOLEAN;
import static pragma.PragmaType.DECIMAL;
import static pragma.PragmaType.DOUBLE;
import static pragma.PragmaType.INT;
import static pragma.PragmaType.LIST;
import static pragma.PragmaType.LONG;
import static pragma.PragmaType.LONG_ARR;
import static pragma.PragmaType.MAP;
import static pragma.PragmaType.OBJECT_ARR;
import static pragma.PragmaType.STRING;
import static pragma.PragmaType.STRING_ARR;
import static pragma.TokenType.*;
...
public class MethodCompiler implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  private final List<PragmaType> locals = new ArrayList<>();
  ...
  private Deque<PragmaType> stack = new ArrayDeque<>();
  ...
  void compile() {
    ...
    if (!methodFunDecl.isStatic) {
      allocateSlot(PragmaType.createInstance(classCompiler.internalName));  // this
    }
    ...
    if (methodFunDecl.functionDescriptor.isAsync) {
      ...
      _loadLocal(continuationVar);
      mv.visitFieldInsn(GETFIELD, "pragma/runtime/Continuation", "methodLocation", "I");
  ...
  @Override public Void visitThrowError(Stmt.ThrowError stmt) {
    mv.visitTypeInsn(NEW, "pragma/runtime/RuntimeError");
    push(ANY);
    dupVal();
    loadConst(stmt.msg);
    compile(stmt.source);
    compile(stmt.offset);
    mv.visitMethodInsn(INVOKESPECIAL, "pragma/runtime/RuntimeError", "<init>", "(Ljava/lang/String;Ljava/lang/String;I)V", false);
    mv.visitInsn(ATHROW);
    pop(5);
    return null;
  }
  ...
  @Override public Void visitRegexSubst(Expr.RegexSubst expr) {
    ...
    loadVar(expr.captureArrVarDecl);
    mv.visitFieldInsn(GETFIELD, "pragma/runtime/RegexMatcher", "matcher", "Ljava/util/regex/Matcher;");
  ...
  @Override public Void visitBinary(Expr.Binary expr) {
    ...
    if (expr.operator.is(INSTANCE_OF,BANG_INSTANCE_OF)) {
      compile(expr.left);
      box();
      emitInstanceof(PragmaType.valueOf(((Expr.Literal)expr.right).value.getType()));
    ...
    if (expr.operator.isBooleanOperator()) {
      ...
      PragmaType operandType = expr.left.type.is(BOOLEAN) ? INT :
  ...
  @Override public Void visitPrefixUnary(Expr.PrefixUnary expr) {
    ...
    if (expr.operator.getType().isType()) {
      convertTo(PragmaType.valueOf(expr.operator.getType()), true, expr.operator);
      return null;
    }
  ...
  @Override public Void visitCall(Expr.Call expr) {
    ...
    if (isFunctionCall) {
      ...
      PragmaType paramType = null;
      for (int i = 0; i < argCount; i++) {
        Expr       arg     = expr.args.get(i);
        PragmaType argType = arg.type;
  ...
  @Override public Void visitMethodCall(Expr.MethodCall expr) {
    ...
    if (expr.methodDescriptor != null) {
      ...
      if (method.returnType.is(ITERATOR) && !expr.isMethodCallTarget) {
        ...
        // to List since Iterators are not standard Pragma types.
        invokeMaybeAsync(true, ANY, 1, expr.location, () -> {},
                         () -> {
                           loadNullContinuation();
                           invokeStatic(RuntimeUtils.class, "convertIteratorToList", Object.class, Continuation.class);
  ...
  private List<PragmaType> methodParamTypes(FunctionDescriptor method) {
    List<PragmaType> types = new ArrayList<>();
  ...
  private void invokeBuiltinFunction(Expr.Call expr, FunctionDescriptor func) {
    ...
    if (invokeWrapper) {
      ...
      List<PragmaType> finalParamTypes = paramTypes;
      ...
      invokeMaybeAsync(func.isAsync, func.returnType, 0, expr.location,
                       () -> {
                         ...
                         for (; argIdx < nonVarArgCount; argIdx++, param++) {
                           ...
                           PragmaType paramType = finalParamTypes.get(param);
                           compile(arg);
  ...
  private void invokeMethodHandle() {
    mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/invoke/MethodHandle", "invokeExact", "(Lpragma/runtime/Continuation;Ljava/lang/String;ILjava/lang/Object;)Ljava/lang/Object;", false);
  ...
  private void newInstance(PragmaType type) {
  ...
  private void emitReturn(PragmaType returnType) {
  ...
  private void invokeMethod(boolean isStatic, String internalClassName, String methodName, PragmaType returnType, List<PragmaType> paramTypes) {
  ...
  private static String getMethodDescriptor(PragmaType returnType, List<PragmaType> paramTypes) {
  ...
  private static List<PragmaType> methodParamTypes(Expr.FunDecl funDecl) {
  ...
  private void push(PragmaType type) {
    stack.push(type);
  }
  ...
  private PragmaType peek() {
    return stack.peek();
  }

  private PragmaType pop() {
    return stack.pop();
  }
   ...
   */
  private void swap() {
    PragmaType type1 = pop();
    PragmaType type2 = pop();
  ...
  private void box(PragmaType type) {
    Utils.box(mv, type);
  }
  ...
  private void unbox(PragmaType type) {
    if (type.isPrimitive()) {
   ...
   */
  private void loadNull(PragmaType type) {
    _loadConst(null);
    push(type);
  }
   ...
   */
  private PragmaType _loadConst(Object obj) {
    return Utils.loadConst(mv, obj);
  }

  private void loadDefaultValue(PragmaType type) {
    switch (type.getType()) {
   ...
   */
  private Object valueOf(PragmaType type, int value) {
    switch (type.getType()) {
  ...
  private Void convertTo(PragmaType type, boolean couldBeNull, SourceLocation location) {
  ...
  private void castToBoxedType(PragmaType type) {
  ...
  private void ifInstanceof(PragmaType type) {
    mv.visitInsn(DUP);
    mv.visitTypeInsn(INSTANCEOF, type.getInternalName());
  }

  private void emitInstanceof(PragmaType type) {
    mv.visitTypeInsn(INSTANCEOF, type.getInternalName());
  }
  ...
  private void throwIfNull(String msg, SourceLocation location) {
    ...
    mv.visitJumpInsn(IFNONNULL, isNotNull);
    _popVal();
    mv.visitTypeInsn(NEW, "pragma/runtime/NullError");
    mv.visitInsn(DUP);
    _loadConst(msg);
    _loadLocation(location);
    mv.visitMethodInsn(INVOKESPECIAL, "pragma/runtime/NullError", "<init>", "(Ljava/lang/String;Ljava/lang/String;I)V", false);
    mv.visitInsn(ATHROW);
  ...
  private void tryCatch(Class exceptionClass, Runnable tryBlock, Runnable catchBlock) {
    ...
    Deque<PragmaType> savedStack = stack;
   ...
   */
  private void invokeMaybeAsync(boolean isAsync, PragmaType returnType, int numArgsAlreadyOnStack, Location location, Runnable compileArgs, Runnable invoker) {
    if (!isAsync) {
      compileArgs.run();
      invoker.run();
      return;
    ...
    mv.visitTypeInsn(ANEWARRAY, "java/lang/Object");
    _storeLocal(objArr);
    Deque<PragmaType> savedStack = new ArrayDeque<>(stack);
    int               slot       = 0;
    for (int i = 0; i < size; i++) {
      final var  isLocalVar = i < numLocals;
      PragmaType type       = isLocalVar ? locals.get(slot) : peek();
    ...
    Runnable restoreStack = () -> {
      // Restore in reverse order
      Iterator<PragmaType> iter = savedStack.descendingIterator();
      for (int i = size - 1; i >= numLocals; i--) {
        PragmaType type = iter.next();
    ...
    List<PragmaType> savedLocals = new ArrayList<>(locals);
    Runnable restoreState = () -> {
      ...
      for (int i = minimumSlot; i < numLocals; i++) {
        PragmaType type = savedLocals.get(slot2);
      ...
      _loadLocal(continuationVar);
      mv.visitFieldInsn(GETFIELD, "pragma/runtime/Continuation", "result", "Ljava/lang/Object;");
      push(ANY);
    ...
    mv.visitTypeInsn(NEW, "pragma/runtime/Continuation");
    mv.visitInsn(DUP);
    _loadLocal(contVar);
    ...
    _loadLocal(longArr);
    _loadLocal(objArr);
    mv.visitMethodInsn(INVOKESPECIAL, "pragma/runtime/Continuation", "<init>", "(Lpragma/runtime/Continuation;Ljava/lang/invoke/MethodHandle;I[J[Ljava/lang/Object;)V", false);
    mv.visitInsn(ATHROW);
  ...
  private void defineVar(Expr.VarDecl var) {
    ...
    PragmaType type = var.isHeapLocal ? HEAPLOCAL : var.type;
  ...
  private void undefineVar(Expr.VarDecl varDecl, Label endBlock) {
    PragmaType type = varDecl.isHeapLocal ? ANY : varDecl.type;
   ...
   */
  private int allocateSlot(PragmaType type) {
    int i;
  ...
  private static int slotsNeeded(PragmaType type) {
    if (type == null) {
      return 1;
    }
  ...
  private void setSlot(int idx, PragmaType type) {
  ...
  private void loadGlobals() {
    loadClassField(classCompiler.internalName, Utils.PRAGMA_GLOBALS_NAME, PragmaType.MAP, false);
  }

  private void loadClassField(String internalClassName, String fieldName, PragmaType type, boolean isStatic) {
    _loadClassField(internalClassName, fieldName, type, isStatic);
    push(type);
  }

  private void _loadClassField(String internalClassName, String fieldName, PragmaType type, boolean isStatic) {
    if (isStatic) {
  ...
  private void loadVar(Expr.VarDecl varDecl) {
    ...
    if (varDecl.isHeapLocal) {
      ...
      invokeVirtual(pragma.runtime.HeapLocal.class, methodName);
      return;
    }
  ...
  private void checkedCast(PragmaType type) {
  ...
  private void _storeLocal(PragmaType type, int slot) {
    if (type.isPrimitive()) {
      switch (type.getType()) {
   ...
   */
  private void loadField(Token accessOperator, boolean createIfMissing, PragmaType defaultType, Object defaultValue) {
  ...
  private Void loadLocal(int slot) {
    ...
    _loadLocal(slot);
    PragmaType type = locals.get(slot);
    push(type);
    return null;
  }
