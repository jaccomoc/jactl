...
import static pragma.TokenType.OBJECT_ARR;
 ...
 */
public class Resolver implements Expr.Visitor<PragmaType>, Stmt.Visitor<Void> {

  private final PragmaContext      pragmaContext;
   ...
   */
  Resolver(PragmaContext pragmaContext, Map<String,Object> globals) {
    this.pragmaContext = pragmaContext;
    this.globals        = globals;
    globals.keySet().forEach(global -> {
      if (!pragmaContext.globalVars.containsKey(global)) {
        Expr.VarDecl varDecl = new Expr.VarDecl(new Token("",0).setType(IDENTIFIER).setValue(global),
                                                null);
        varDecl.type = PragmaType.typeOf(globals.get(global));
        varDecl.isGlobal = true;
        pragmaContext.globalVars.put(global, varDecl);
      }
    });
  ...
  @Override public PragmaType visitRegexMatch(Expr.RegexMatch expr) {
    expr.isConst = false;
  ...
  @Override public PragmaType visitRegexSubst(Expr.RegexSubst expr) {
    visitRegexMatch(expr);
    resolve(expr.replace);
    return expr.type = STRING;
  }

  @Override public PragmaType visitBinary(Expr.Binary expr) {
    resolve(expr.left);
    resolve(expr.right);
    ...
    expr.type = PragmaType.result(expr.left.type, expr.operator, expr.right.type);
    if (expr.isConst) {
  ...
  @Override public PragmaType visitTernary(Expr.Ternary expr) {
    ...
    return expr.type = PragmaType.result(expr.second.type, expr.operator1, expr.third.type);
  }

  @Override public PragmaType visitPrefixUnary(Expr.PrefixUnary expr) {
    resolve(expr.expr);
    if (expr.operator.getType().isType()) {
      // Type case so if we already know we have a bad cast then throw error
      if (!expr.expr.type.isConvertibleTo(PragmaType.valueOf(expr.operator.getType()))) {
      ...
      return expr.type = PragmaType.valueOf(expr.operator.getType());
    }
  ...
  @Override public PragmaType visitPostfixUnary(Expr.PostfixUnary expr) {
    resolve(expr.expr);
    expr.isConst      = expr.expr.isConst;
  ...
  @Override public PragmaType visitListLiteral(Expr.ListLiteral expr) {
    expr.exprs.forEach(this::resolve);
    return expr.type = LIST;
  }

  @Override public PragmaType visitMapLiteral(Expr.MapLiteral expr) {
  ...
  @Override public PragmaType visitVarDecl(Expr.VarDecl expr) {
    ...
    PragmaType type = resolve(expr.initialiser);
    if (expr.type == null) {
  ...
  @Override public PragmaType visitFunDecl(Expr.FunDecl expr) {
  ...
  private PragmaType doVisitFunDecl(Expr.FunDecl expr, boolean defineVar) {
    ...
    try {
      ...
      List<PragmaType> paramTypes = expr.parameters.stream()
  ...
  @Override public PragmaType visitIdentifier(Expr.Identifier expr) {
  ...
  @Override public PragmaType visitLoadParamValue(Expr.LoadParamValue expr) {
    Expr.VarDecl varDecl = lookup(expr.paramDecl.name);
    expr.varDecl = varDecl;
    return expr.type = expr.varDecl.type;
  }

  @Override public PragmaType visitVarAssign(Expr.VarAssign expr) {
  ...
  @Override public PragmaType visitVarOpAssign(Expr.VarOpAssign expr) {
    expr.type = resolve(expr.identifierExpr);
  ...
  @Override public PragmaType visitNoop(Expr.Noop expr) {
    expr.isConst = false;
    // Type will already have been set by parent (e.g. in visitVarOpAssign)
    return expr.type;
  }

  @Override public PragmaType visitAssign(Expr.Assign expr) {
    return resolveFieldAssignment(expr, expr.parent, expr.field, expr.expr, expr.accessType);
  }

  @Override public PragmaType visitOpAssign(Expr.OpAssign expr) {
    if (expr.expr instanceof Expr.Binary) {
      Expr.Binary valueExpr = (Expr.Binary) expr.expr;
  ...
  private PragmaType resolveFieldAssignment(Expr expr, Expr parent, Expr field, Expr valueExpr, Token accessType) {
    resolve(parent);
  ...
  @Override public PragmaType visitExprString(Expr.ExprString expr) {
    expr.exprList.forEach(this::resolve);
    return expr.type = STRING;
  }

  @Override
  public PragmaType visitReturn(Expr.Return returnExpr) {
  ...
  @Override public PragmaType visitPrint(Expr.Print printExpr) {
    resolve(printExpr.expr);
    return printExpr.type = BOOLEAN;
  }

  @Override public PragmaType visitClosure(Expr.Closure expr) {
    resolve(expr.funDecl);
    return expr.type = FUNCTION;
  }

  @Override public PragmaType visitCall(Expr.Call expr) {
  ...
  @Override public PragmaType visitMethodCall(Expr.MethodCall expr) {
    resolve(expr.parent);
    ...
    if (!expr.parent.type.is(ANY)) {
      ...
      if (descriptor != null) {
        ...
        for (int i = 0; i < expr.args.size(); i++) {
          ...
          PragmaType paramType = descriptor.paramTypes.get(i);
  ...
  @Override public PragmaType visitArrayLength(Expr.ArrayLength expr) {
    resolve(expr.array);
    return expr.type = INT;
  }

  @Override public PragmaType visitArrayGet(Expr.ArrayGet expr) {
    resolve(expr.array);
    resolve(expr.index);
  ...
  private PragmaType evaluateConstExpr(Expr.Binary expr) {
    ...
    switch (expr.type.getType()) {
      ...
      case DECIMAL: {
        ...
        expr.constValue = RuntimeUtils.decimalBinaryOperation(left, right, RuntimeUtils.getOperatorType(expr.operator.getType()), pragmaContext.maxScale,
                                                              expr.operator.getSource(), expr.operator.getOffset());
        break;
      }
    }
...
//    newVarDecl.isPassedAsHeapLocal = true;
  ...
  private static Object incOrDec(boolean isInc, PragmaType type, Object val) {
  ...
  private void declare(Token name) {
    ...
    if (!pragmaContext.replMode || !isAtTopLevel()) {
  ...
  private void define(Token name, Expr.VarDecl decl) {
    ...
    if (!decl.isParam && isAtTopLevel() && pragmaContext.replMode) {
      decl.isGlobal = true;
  ...
  private Map<String,Expr.VarDecl> getVars() {
    ...
    if (pragmaContext.replMode && isAtTopLevel()) {
      return pragmaContext.globalVars;
    }
  ...
  private Expr.VarDecl lookup(String name, Token location, boolean functionLookup, boolean existenceCheckOnly) {
    ...
    if (varDecl == null) {
      varDecl = pragmaContext.globalVars.get(name);
    }
   ...
   */
  private Stmt explicitReturn(Stmt stmt, PragmaType returnType) {
      return doExplicitReturn(stmt, returnType);
  }

  private Stmt doExplicitReturn(Stmt stmt, PragmaType returnType) {
  ...
  private Expr.FunDecl createWrapperFunction(Expr.FunDecl funDecl) {
    ...
    BiFunction<String, PragmaType,Stmt.VarDecl> createParam =
      (name,type) -> {
    ...
    ifStmts.stmts.add(varDecl(wrapperFunDecl, argArrName, PragmaType.OBJECT_ARR, argsIdent, startToken));
  ...
  private Stmt.VarDecl varDecl(Expr.FunDecl ownerFunDecl, String name, PragmaType type, Expr init, Token token) {
    return varDecl(ownerFunDecl, new Token(IDENTIFIER, token).setValue(name), type, init);
  }

  private Stmt.VarDecl varDecl(Expr.FunDecl ownerFunDecl, Token name, PragmaType type, Expr init) {
  ...
  private Stmt.Return returnStmt(Token token, Expr expr, PragmaType type) {
