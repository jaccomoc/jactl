...
class CompilerTest {
  ...
  private void doTest(String code, boolean evalConsts, boolean replMode, boolean testAsync, Object expected)  {
    ...
    try {
      PragmaContext pragmaContext = PragmaContext.create()
                                                 .evaluateConstExprs(evalConsts)
                                                 .replMode(replMode)
                                                 .debug(debug)
                                                 .build();
      Object result = Compiler.run(code, pragmaContext, testAsync, createGlobals());
  ...
  private void doTestError(String code, boolean evalConsts, boolean replMode, String expectedError) {
    try {
      PragmaContext pragmaContext = PragmaContext.create()
                                                 ...
                                                 .debug(debug)
                                                 .build();
      Compiler.run(code, pragmaContext, new HashMap<>());
      fail("Expected PragmaError");
    }
    catch (PragmaError e) {
...
//      globals.put("timestamp", handle);
  ...
  @Test public void globals() {
    ...
    BiConsumer<String,Object> runtest = (code,expected) -> {
      Object result = Compiler.run(code, pragmaContext, globals);
      assertEquals(expected, result);
    };
...
//    debug=true;
  ...
  private Function<Map<String,Object>,Future<Object>> compile(String code) {
    PragmaContext pragmaContext = PragmaContext.create()
                                               ...
                                               .build();
    Map<String,Object> globals  = new HashMap<>();
    return Compiler.compile(code, pragmaContext, globals);
  }
}