...
//  }
  ...
  @Test public void contextsWithSeparateBuiltIns() throws NoSuchMethodException {
    ...
    assertEquals(true, Jactl.eval("TestClass__LocalDate4.now().lengthOfYear() in [365,366]", new HashMap<>(), context));
    
    JactlContext context2 = JactlContext.create().build();
    try {
      Jactl.eval("TestClass__LocalDate4.now().lengthOfYear() in [365,366]", new HashMap<>(), context2);
      fail("Should have thrown a CompileError exception");
    }
    catch (CompileError e) {
      assertTrue(e.getMessage().contains("Reference to unknown variable 'TestClass__LocalDate4"));
    }
    
    try {
      context2.createClass("test.jactl.time.XXX");
    }
  ...
  public static class MyLocalDate5 implements ChronoLocalDate {
    private LocalDate date;
    private MyLocalDate5(LocalDate date) { this.date = date; }
    public static MyLocalDate5 now() { return new MyLocalDate5(LocalDate.now()); }
    public int getYear() { return date.getYear(); }
    public int getMonthValue() { return date.getMonthValue(); }
    public int getDayOfMonth() { return date.getDayOfMonth(); }
    public int lengthOfYear() { return date.lengthOfYear(); }
    public boolean isAfter(ChronoLocalDate other) { return this.date.isAfter(((MyLocalDate5)other).date); }
    public MyLocalDate5 plusDays(long days) { return new MyLocalDate5(date.plusDays(days)); }
    public static MyLocalDate5 of(int year, int month, int dayOfMonth) { return new MyLocalDate5(LocalDate.of(year, month, dayOfMonth)); }
    public static MyLocalDate5 parse(CharSequence text) { return new MyLocalDate5(LocalDate.parse(text)); }
    @Override public String toString() { return date.toString(); }
    @Override public Chronology getChronology() { return null; }
    @Override public int lengthOfMonth() { return 0; }
    @Override public long until(Temporal endExclusive, TemporalUnit unit) { return 0; }
  ...
  @Test public void addFunctionToNewClass() throws NoSuchMethodException {
    Jactl.createClass("test.jactl.time.LocalDate4")
         .javaClass(MyLocalDate5.class)
         .autoImport(true)
         .mapType(ChronoLocalDate.class, MyLocalDate5.class)
         .method("isAfter", "isAfter", "other", ChronoLocalDate.class)
         .method("getYear", "getYear")
         .method("lengthOfYear", "lengthOfYear")
         .method("now", "now")
         ...
         .methodCanThrow("plusDays", "plusDays", "days", long.class)
         .checkpoint((checkpointer,obj) ->  {
           MyLocalDate5 d = (MyLocalDate5) obj;
           checkpointer.writeCInt(d.getYear());
           checkpointer.writeCInt(d.getMonthValue());
           checkpointer.writeCInt(d.getDayOfMonth());
         })
         .restore(restorer -> MyLocalDate5.of(restorer.readCInt(), restorer.readCInt(), restorer.readCInt()))
         .register();

    JactlType type = JactlContext.typeFromClass(MyLocalDate5.class, (JactlContext)null);
  ...
  public static int testLocalDateMethod(MyLocalDate5 localDate) {
    return 3;
  }
}
